...
以下文件是编写本项目代码时可参考的知识文档, 放在 `知识` 文件夹中. 当任务涉及对应主题时, 你应该先阅读相关文档再动手:

- `知识/EJS实战指南_2026_ZOD版.md`: EJS 提示词模板 (SillyTavern 的 ST-Prompt-Template 扩展) 的完整工作流指南, 包括基础语法、`getvar`/`print`/`getwi` 等函数、`<%= %>` 变量填入、世界书操作、装饰器、内容注入、@INJECT、激活正则、多阶段人设系统实战、内置常量和函数速查等. 当任务涉及提示词模板 (`@types/iframe/exported.ejstemplate.d.ts` 相关接口) 或需要编写 EJS 模板代码时, 必须参考本文档.
